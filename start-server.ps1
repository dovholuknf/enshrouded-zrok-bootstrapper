...
$RESERVED_SHARE = (($zid -replace '[^a-zA-Z0-9]', '') + "ENSHROUDED").ToLower()
...
if ($targetEnvironment) {
    ...
    if ($shares) {
        ...
        Invoke-Expression "$PATH_TO_ZROK reserve private ${ENSHROUDED_SERVER_IP}:${ENSHROUDED_PORT} --backend-mode udpTunnel --unique-name $RESERVED_SHARE"
    }
} else {
...
Write-Host "Make sure the dedicated server is running and on port: $ENSHROUDED_PORT!!!"
